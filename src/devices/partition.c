...
static void
read_partition_table(struct block *block, block_sector_t sector, block_sector_t primary_extended_sector, int *part_nr)
{
    ...
    for (i = 0; i < sizeof pt->partitions / sizeof *pt->partitions; i++) {
                 ...
                 || e->type == 0xc5) /* DR-DOS extended partition. */
        {
            ...
            if (sector == 0)
                ...
                read_partition_table(block, e->offset + primary_extended_sector, primary_extended_sector, part_nr);
        }
        else {
            ++*part_nr;
...
static void
found_partition(struct block *block, uint8_t part_type, block_sector_t start, block_sector_t size, int part_nr)
{
