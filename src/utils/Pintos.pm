...
sub read_loader {
    ...
    if (!defined $name) {
        ...
        # Build search paths with current project prioritized
        my @search_paths;
        if (defined $project) {
            # First try current project's loader (both relative and absolute)
            push @search_paths, "../$project/build/loader.bin";
            push @search_paths, "/home/pintos/pintos/src/$project/build/loader.bin";
        }
        # Then try other projects in logical order
        foreach my $proj (qw(userprog vm filesys threads)) {
            next if defined $project && $proj eq $project;
            push @search_paths, "../$proj/build/loader.bin";
            push @search_paths, "/home/pintos/pintos/src/$proj/build/loader.bin";
        }
